...
MIRO_SPEECH_PATH = "./chatgptresult.mp3"
MIRO_SPEECH_FILE = "chatgptresult.mp3"
...
class Streamer():
    ...
    hotKeys = ["Hello GPT", "Thank you GPT"]
    responses = {
            "Hello GPT" : "Hi,What can i do for you?",
            "Thank you GPT" : "Have a nice day!",
            #this will be wherever the intro file is locate
            }
    ...
    def recordSound(self):
        ...
        rospy.Subscriber(topic, Int16MultiArray,
                self.callBackMics, queue_size=1, tcp_nodelay=True)
    ...
    def talkToMiRoOnline(self):
        ...
        """
        from chatbot import chat_response_with_key_words
        self.saveAudioMics()
        input_file_name = './voice.mp3'
        response_file_name = './chatgptresult.wav'
        self.current_state,self.history_messages = chat_response_with_key_words(input_file_name,response_file_name,self.current_state,self.history_messages)
        if self.current_state == 'chatting':
            self.speakMiRo(None, response_file_name)
    ...
    def speakMiRo(self, inGttsObj, savePath):
        ...
        #print(savePath.split('/')[-1] ,savePath)
    ...
    def callBackMics(self, msg):
        ...
        if not self.__micBuff is None:
            ...
            self.__micBuff = self.__micBuff[-SAMPLE_COUNT: ]
            ...
            if self.__micBuff.shape[0] >= SAMPLE_COUNT - 1:
                #ending the recording 
                self.__outBuff = self.__micBuff
                #self.__micBuff = self.__micBuff[-SAMPLE_COUNT: ]
                #print(self.__micBuff)
                #print("OK!")
...
if __name__ == "__main__":
    ...
    while not rospy.core.is_shutdown():
        ...
        elif soundInterface.recordAudio:
            #soundInterface.recordSound()
            soundInterface.saveAudioMics()
            #soundInterface.recordAudio= False
            soundInterface.clearRecording()
